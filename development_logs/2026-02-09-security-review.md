...
- **ステータス:** ✅ 対応済み (2026-02-10) - RSA-SHA256 署名検証と `b=` 除去後のヘッダー再正規化を実装
...
- **ステータス:** ✅ 対応済み (2026-02-10) - SMTP `tls_enabled` のデフォルトを `true` に変更
...
- **ステータス:** ⚠️ 部分対応済み (2026-02-10) - IMAP `STARTTLS` / POP3 `STLS` を実装 (TLS必須化は未対応)
...
## Next Steps
1. ~~Phase 1のCRITICAL脆弱性の修正を最優先で実施~~ → Phase 1 完了
